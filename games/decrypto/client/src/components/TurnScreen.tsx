...
function HeaderPanel({
  ...
  const transmissionTeams = isThreePlayerMode(room.gameMode, room.threePlayer)
    ? [room.threePlayer.encryptorTeam]
    : (['red', 'blue'] as TeamId[]);
            ...
            {transmissionTeams.map((team) => {
              ...
              return (
                <span key={team} className={`rounded-lg border ${TEAM_STYLES[team].border} px-2 py-1`}>
                  <span className={TEAM_STYLES[team].text}>
...
function TeamClueStatus() {
  const room = useGameStore((s) => s.room)!;
  const turn = room.turn!;
  const teams = isThreePlayerMode(room.gameMode, room.threePlayer)
    ? [room.threePlayer.encryptorTeam]
    : (['red', 'blue'] as TeamId[]);
