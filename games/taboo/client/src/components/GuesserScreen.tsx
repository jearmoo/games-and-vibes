  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-6 gap-6 animate-fade-in">
      ...
      <div className="glass-card rounded-2xl p-6 border border-white/5 w-full max-w-xs text-center">
        ...
        <div className="text-gray-400 text-sm">Team {cluingTeam} is cluing</div>
      </div>
      ...
      {correctCards.length > 0 && (
        <div className="w-full max-w-xs space-y-1.5">
          ...
          {correctCards.map((card, i) => (
            <div
              key={i}
              className="px-3 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-emerald-300 text-sm font-medium"
            >
      ...
      <div className="text-gray-500 text-sm">
        ...
        {totalBuzzes > 0 && (
          <span className="text-team-b-glow">
            {' '}
            · {totalBuzzes} buzz{totalBuzzes !== 1 ? 'es' : ''}
          </span>
        )}
      </div>
      ...
      {buzzedWords.length > 0 && (
        <div className="flex flex-wrap gap-1.5 justify-center">
          {buzzedWords.map(([word, count]) => (
            <span
              key={word}
              className="px-2 py-1 bg-team-b/10 text-team-b-glow/80 rounded-lg text-xs border border-team-b/15"
            >
              {word}
              {count > 1 && ` ×${count}`}
            </span>
