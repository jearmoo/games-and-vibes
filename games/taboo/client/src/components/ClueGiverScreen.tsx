          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                {card.result === 'correct' && <span className="text-emerald-400/60 text-sm font-display">+3</span>}
              </div>
              {card.result === 'correct' ? (
                <button
        ...
        <div className="glass-card rounded-xl p-3 border border-team-b/20">
          ...
          <div className="flex flex-wrap gap-1.5">
            {buzzedWords.map(([word, count]) => (
              <span
                key={word}
                className="px-2.5 py-1 bg-team-b/15 text-team-b-glow rounded-lg text-sm border border-team-b/20"
              >
                {word}
                {count > 1 && <span className="text-[10px] ml-1">×{count}</span>}
              </span>
            ))}
          </div>
        </div>
