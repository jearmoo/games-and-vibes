    ...
    <div className="w-full">
      <div
        className={`text-center font-display text-5xl tracking-wider ${
          isFinal
            ? 'text-team-b-glow animate-buzz-shake'
            : isCritical
              ? 'text-team-b-glow animate-pulse'
              : isLow
                ? 'text-team-b-glow animate-pulse-slow'
                : 'text-white'
        }`}
      >
        {remaining}
      ...
      <div className="w-full h-1 bg-surface-raised rounded-full mt-2 overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-200 ${isLow ? 'bg-team-b' : 'bg-indigo-500'}`}
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
