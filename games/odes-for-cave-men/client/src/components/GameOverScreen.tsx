...
function Confetti({ color }: { color: 'A' | 'B' }) {
  ...
  useEffect(() => {
    ...
    const colors =
      color === 'A'
        ? ['#d97706', '#f59e0b', '#fbbf24', '#fde68a', '#ffffff']
        : ['#059669', '#10b981', '#34d399', '#6ee7b7', '#ffffff'];
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-6 gap-8 animate-fade-in">
      ...
      <div className="fixed inset-0 pointer-events-none">
        {winner === 'A' && <div className="absolute inset-0 bg-amber-500/15" />}
        {winner === 'B' && <div className="absolute inset-0 bg-emerald-500/15" />}
      </div>
      ...
      <div className="text-center relative z-10 animate-score-pop">
        <h1
          ...
          style={{
            textShadow: winner
              ? `0 0 40px ${winner === 'A' ? 'rgba(217,119,6,0.4)' : 'rgba(5,150,105,0.4)'}`
              : undefined,
          }}
        >
