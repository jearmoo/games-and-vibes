...
export default function Timer({ endTime, duration = 90 }: { endTime: number; duration?: number }) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="w-full h-1 bg-surface-raised rounded-full mt-2 overflow-hidden">
        <div
          className={`h-full rounded-full transition-all duration-200 ${isLow ? 'bg-red-500' : 'bg-amber-500'}`}
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
