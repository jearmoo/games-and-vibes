import type { CompReviewCard, RoundEntry } from '../../compStore';

export function resultIcon(card: CompReviewCard): { icon: string; color: string } {
...
export function pointColor(pts: number): string {
...
interface RoundCardProps {
  entry: RoundEntry;
  roundNum: number;
  /** Full round history up to and including this round, used to compute cumulative score */
  roundHistory: RoundEntry[];
}

export default function RoundCard({ entry, roundNum, roundHistory }: RoundCardProps) {
  const isPositive = entry.score >= 0;

  // Cumulative score for this cluer through this round
  const cumulative = roundHistory
    .slice(0, roundNum)
    .filter((r) => r.cluerName === entry.cluerName)
    ...
    <div className="rounded-xl border border-white/5 overflow-hidden bg-surface-card">
      ...
      <div className="flex items-center justify-between px-3.5 py-2 border-b border-white/5 bg-white/[0.02]">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] text-gray-500">({cumulative} total)</span>
        </div>
      </div>
        ...
        {entry.cards.map((card, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-1.5 text-[11px] leading-tight py-0.5">
              ...
              <span
                className={`truncate text-[9px] ${isCorrect ? 'text-gray-500' : 'text-gray-600 line-through decoration-gray-800'}`}
              >
                {card.word3}
              </span>
              {wasAdjusted && (
