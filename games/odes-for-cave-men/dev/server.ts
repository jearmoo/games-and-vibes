...
app.post('/api/generate', async (req, res) => {
  try {
    const { count, difficulty } = req.body as { count: number; difficulty: 'gray' | 'red' };
    const existingWords = loadExistingWords();
    const result = await generateCards({ count, difficulty, existingWords });
    const cards = result.cards.map((card) => ({
      ...card,
      duplicate: existingWords.has(card.word1.toLowerCase()),
    }));
    res.json({ cards });
  } catch (error) {
...
app.post('/api/keep', (req, res) => {
  try {
    ...
    const data = JSON.parse(raw) as { game_data: Array<Record<string, string>> };
