    ...
    <div className="h-full flex flex-col p-4 gap-3 animate-fade-in">
          ...
          <div className="flex flex-wrap gap-1.5">
            {teamPlayers
              ...
              .map((p) => (
                ...
                >
                  {p.name} {p.id === me?.id && <span className="text-gray-400 text-[10px]">(you)</span>}
                </button>
              ))}
          </div>
        ) : (
      ...
      <div className="glass-card rounded-xl p-3 border border-white/5">
        ...
        {wordsLoading ? (
          <div className="flex items-center gap-2 py-1">
            ...
            <span className="text-gray-300 text-sm">Fetching words...</span>
          </div>
        ) : (
                ...
                {isTM && (
                  <button
                    ...
                    className={`text-xs transition-colors ${
                      refreshingIdx === i ? 'text-accent animate-spin' : 'text-gray-500 hover:text-accent'
                    } disabled:opacity-50`}
                  >
                    ↻
                  </button>
                )}
                {i < challengeCards.length - 1 && <span className="text-gray-600 mx-0.5">·</span>}
              </div>
            ))}
          </div>
        )}
      ...
      <div className="text-center text-xs">
        {isTM ? (
          ...
          <span className="text-gray-300">Suggest taboo words</span>
        )}
      </div>
            ...
            >
              ...
              {isTM && !isLocked && (
                <button
                  ...
                  className="text-team-b-glow/50 hover:text-white text-xs"
                >
                  &times;
                </button>
              )}
          ...
          >
            {!ownClueGiverId
              ...
              : tabooSuggestions.length !== settings.maxTabooWords
                ? `Need exactly ${settings.maxTabooWords} words (${tabooSuggestions.length}/${settings.maxTabooWords})`
                : 'Lock In'}
          </button>
        )
      ) : (
        <div
          className={`w-full py-3 rounded-2xl text-center font-display tracking-wider text-sm ${
            isLocked
              ...
              : 'border border-white/5 text-gray-300'
          }`}
        >
    ...
    >
      <div className="flex items-center justify-between px-3 py-2.5">
        <div className="flex items-center gap-2 text-xs">
          <span className="text-gray-300">Other team</span>
          {ready ? (
            <span className="text-emerald-400 font-semibold text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-400/10 border border-emerald-400/20">
              Ready
            </span>
          ) : (
      ...
      {expanded && (
        <div className="px-3 pb-2.5 pt-0 border-t border-white/[0.03] mt-0">
          <div className="flex items-center justify-between text-[11px] pt-2">
            <span className="text-gray-500">Clue-giver</span>
            <span className={hasClueGiver ? 'text-emerald-400' : 'text-gray-600'}>
              {hasClueGiver ? 'Picked' : 'Not yet'}
            </span>
          </div>
          <div className="flex items-center justify-between text-[11px] mt-1">
            <span className="text-gray-500">Taboo words</span>
            <span className="text-gray-300">{tabooCount}</span>
          </div>
        </div>
      )}
    </button>
