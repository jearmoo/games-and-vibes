import { useGameStore, useTeamName, type RoundArchiveEntry, type TeamRoundData } from '../store';
    ...
    <div className="flex-1 p-3 min-w-0 space-y-2">
      ...
      <div>
        ...
        <div className="text-[10px] text-gray-500 mt-0.5 truncate">
          <span className="text-gray-400">{data.clueGiverName}</span> cluing
        </div>
      </div>
      ...
      <div className="space-y-0.5">
        {data.cards.map((card, i) => (
          <div key={i} className="flex items-center gap-1.5 text-[11px] leading-tight">
            <span
              className={`w-3 text-center shrink-0 ${card.result === 'correct' ? 'text-emerald-400' : 'text-gray-600'}`}
            >
              {card.result === 'correct' ? '\u2713' : '\u2022'}
            </span>
            <span
              className={`truncate ${card.result === 'correct' ? 'text-gray-200' : 'text-gray-500 line-through decoration-gray-800'}`}
            >
              {card.word}
            </span>
          </div>
      ...
      {data.tabooWords.length > 0 && (
        <div>
          <div className="text-[9px] uppercase tracking-wider text-gray-600 mb-1">
            Taboo <span className="normal-case text-gray-500">by {data.tabooMasterName}</span>
          </div>
